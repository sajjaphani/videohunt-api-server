...
function getFeedPagination(query, data, fieldName, paginationFor) {
    if(data.length == 0)
        return {}
    const POST_GET_BASE = API_BASE + paginationFor + '/'
    let pagination = {}
    if (query.query == QUERY_BY_DATE_RANGE) {
        ...
        pagination.previous = POST_GET_BASE + '?limit=' + query.limit + "&since=" + data[0][fieldName].getTime()
        if (data.length == query.limit + 1) {
            let lastFeed = data.pop()
            pagination.next = POST_GET_BASE + '?limit=' + query.limit + "&until=" + lastFeed[fieldName].getTime()
        }
    } else if (query.query == QUERY_BY_DATE_FORWARD) {
        // Forward query, (E.g, give me 3 feeds, starts at since
        // We may have prev page here
        pagination.next = POST_GET_BASE + '?limit=' + query.limit + "&until=" + data[0][fieldName].getTime()
        if (data.length == query.limit + 1) {
            let lastFeed = data.pop()
            pagination.previous = POST_GET_BASE + '?limit=' + query.limit + "&since=" + lastFeed[fieldName].getTime()
        }
    } else {
